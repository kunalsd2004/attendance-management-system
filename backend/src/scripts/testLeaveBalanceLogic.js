...
async function testLeaveBalanceLogic() {
  try {
    // Connect to database
    await connectDB();
    console.log('Connected to database');

    console.log('\n=== TESTING NEW LEAVE BALANCE LOGIC ===');
    ...
    // Deduct balance after final approval
    const userBalance = testUser.leaveBalances.find(
      balance => balance.leaveType.toString() === leaveType._id.toString() && balance.year === currentYear
    );
    ...
    const expectedUsed = initialBalances.find(b => b.leaveType.toString() === leaveType._id.toString())?.used || 0;
    ...
    console.log('\n✅ Leave balance logic test completed successfully!');
    process.exit(0);
