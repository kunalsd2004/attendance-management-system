  ...
  try {
    ...
    if (usersWithDepartments.length > 0) {
      usersWithDepartments.forEach(user => {
        console.log(`  - ${user.profile.firstName} ${user.profile.lastName} (${user.employeeId}) - Role: ${user.role} - Department: ${user.profile.department.name}`);
      });
    ...
    verifyUsers.forEach(user => {
      ...
      console.log(`  - ${user.profile.firstName} ${user.profile.lastName} (${user.employeeId}) - Role: ${user.role} - Department: ${deptName}`);
    });
