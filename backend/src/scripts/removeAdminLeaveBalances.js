...
async function removeAdminLeaveBalances() {
  try {
    ...
    for (const user of adminUsers) {
      console.log(`\nProcessing admin user: ${user.profile.firstName} ${user.profile.lastName} (${user.employeeId})`);
    ...
    verifyUsers.forEach(user => {
      console.log(`  - ${user.profile.firstName} ${user.profile.lastName} (${user.employeeId}): ${user.leaveBalances.length} leave balances`);
    });
