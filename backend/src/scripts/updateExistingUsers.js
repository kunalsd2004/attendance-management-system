...
async function updateExistingUsers() {
  try {
    ...
    for (const user of users) {
      console.log(`\nProcessing user: ${user.profile.firstName} ${user.profile.lastName} (${user.employeeId}) - Role: ${user.role}`);
    ...
    verifyUsers.forEach(user => {
      ...
      console.log(`  - ${user.profile.firstName} ${user.profile.lastName} (${user.employeeId}): ${deptName} (${deptCode}) - ${leaveCount} leave balances`);
    });
