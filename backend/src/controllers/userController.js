...
const createUser = async (req, res) => {
  try {
    ...
    if (!employeeId || !email || !password || !firstName || !lastName || !designation || !joiningDate || !role) {
    ...
    if (existingUser) {
      return res.status(400).json(formatErrorResponse('User with this email or employee ID already exists'));
    }
    ...
    logger.info(`New user created successfully: ${createdUser.employeeId} (${createdUser.email})`);
...
const resetUserPassword = async (req, res) => {
  try {
    ...
    logger.info(`Password reset for user: ${user.employeeId} (${user.email})`);
