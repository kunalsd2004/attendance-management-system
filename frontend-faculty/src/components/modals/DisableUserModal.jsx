...
const DisableUserModal = ({ user, isOpen, onClose, onUserDisabled }) => {
  ...
  const handleDisableUser = async () => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        onUserDisabled(); // Refresh the user list
        onClose();
      } else {
        toast.error(response?.message || 'Failed to disable user');
      }
    } catch (error) {
      console.error('Error disabling user:', error);
      toast.error('Error disabling user: ' + (error.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        ...
        <div className="mb-6">
          <p className="text-gray-600 mb-4">
            Are you sure you want to disable this user? This action will:
          </p>
          <ul className="text-sm text-gray-600 space-y-2 mb-4">
            ...
            <li className="flex items-start gap-2">
              <span className="text-red-500 mt-1">•</span>
              Hide the user from active user lists
            </li>
            <li className="flex items-start gap-2">
              <span className="text-yellow-500 mt-1">•</span>
              The user can be re-enabled later if needed
            </li>
          </ul>
